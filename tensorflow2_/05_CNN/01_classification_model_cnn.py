...
"""
    卷积层
        -> keras.layers.Conv2D()
            -> filters: 输出有多少通道 有多少卷积和
            -> kernel_size: 卷积和大小
            -> padding: 是否需要加 pdding 使得输入输出大小一样
            -> input_shape: 输入图像 size (长 * 宽 * 通道数)
                ** 只有第一层 卷积需要规定， 其他的会自动计算
"""
...
"""
    池化层
        -> keras.layers.MaxPool2D()
            -> pool_size: size 会与步长相等, 所以设置一个即可
            
        -> 经过 Pool 通常会把 Conv2D() 的 filter 翻倍 减少数据损失
"""
...
"""
    activation='relu' -> 0.988
    activation='selu' -> 0.991 图像显示出的训练过程也更好
"""
